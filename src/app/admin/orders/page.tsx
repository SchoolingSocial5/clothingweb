...
export default function OrdersPage() {
  ...
  return (
    <div className="p-[10px] md:p-8 w-full">
      <AdminPageHeader
        title={viewingTrash ? "Orders Trash" : "Orders"}
        description={viewingTrash ? "View and restore soft-deleted orders or permanently delete them" : "Manage and track all customer orders"}
                    ...
                    <td className="px-4 py-5" onClick={e => e.stopPropagation()}>
                      {viewingTrash ? (
                        <div className="flex items-center justify-center gap-2">
                            ...
                            onClick={async () => {
                              if (window.confirm("Are you sure you want to restore this order?")) {
                                try {
                                  ...
                                  setToast({ message: err.message || 'Failed to restore order', type: 'error' });
                                }
                              }
                            }}
                          ...
                          <button
                            onClick={async () => {
                              if (window.confirm("WARNING: Are you sure you want to permanently delete this order? This action CANNOT be undone.")) {
                                try {
                                  await deleteOrderPermanent(order.id);
                                  setToast({ message: 'Order permanently deleted!', type: 'success' });
                                } catch (err: any) {
                                  setToast({ message: err.message || 'Failed to permanently delete order', type: 'error' });
                                }
                              }
                            }}
          ...
          <div className="flex items-center gap-8">
            ...
            {viewingTrash ? (
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={async () => {
                    if (window.confirm(`Are you sure you want to restore ${selectedOrderIds.length} selected orders?`)) {
                      try {
                        await bulkRestoreOrders(selectedOrderIds);
                        setToast({ message: 'Bulk restore successful!', type: 'success' });
                      } catch (err: any) {
                        setToast({ message: err.message || 'Failed to restore orders', type: 'error' });
                      }
                    }
                  }}
      ...
      <DeleteConfirmModal
        ...
        onConfirm={async () => {
          try {
            if (viewingTrash) {
              ...
              setToast({ message: 'Orders permanently deleted!', type: 'success' });
            } else {
              await useOrderStore.getState().bulkDeleteOrders(selectedOrderIds);
              setToast({ message: 'Orders moved to trash!', type: 'success' });
            }
          } catch (err: any) {
        ...
        title={viewingTrash ? "Permanently Delete Orders" : "Delete Orders"}
        message={viewingTrash
          ? `WARNING: Are you sure you want to permanently delete ${selectedOrderIds.length} selected orders? This action CANNOT be undone.`
          : `Are you sure you want to delete ${selectedOrderIds.length} selected order${selectedOrderIds.length !== 1 ? 's' : ''}? They will be moved to the trash.`
        }
      />
