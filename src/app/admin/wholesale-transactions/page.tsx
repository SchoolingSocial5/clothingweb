...
export default function WholesaleTransactionsPage() {
  ...
  const handleAddToCart = (product: WholesaleProduct) => {
    ...
    if (existing) {
      ...
      setCartItems([...cartItems, { ...product, quantity: product.min_order_quantity || 1, price: parseFloat(product.price) }]);
    }
  };
    ...
    <div className="p-[10px] md:p-8 w-full">
      ...
      <div className="mb-4 flex items-center justify-between px-6 py-4 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm">
        <label className="flex items-center gap-4 cursor-pointer group">
          ...
          />
          <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-black transition-colors">Select All Wholesale Records</span>
        </label>
        <div className="text-[10px] font-black uppercase tracking-widest text-gray-400">
          Showing {orders.length} of {pagination.total} Wholesale Transactions
        </div>
      </div>
              ...
              {orders.length === 0 && !loading && (
                <tr>
                  <td colSpan={user?.position === 'Director' ? 10 : 9} className="px-6 py-20 text-center">
                    ...
                    <p className="text-gray-400 font-bold uppercase tracking-widest text-sm">No wholesale transactions yet</p>
                  </td>
                </tr>
              )}
                ...
                >
                  ...
                  {user?.position === 'Director' && (
                    <td className="px-4 py-5 text-right" onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => {
                          if (window.confirm('Are you sure you want to delete this wholesale transaction?')) {
                            useWholesaleOrderStore.getState().deleteOrder(order.id);
                          }
                        }}
