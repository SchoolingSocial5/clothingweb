...
interface WholesaleProductPickerModalProps {
  onClose: () => void;
  onAddToCart: (p: WholesaleProduct) => void;
  onUpdateQty: (id: number, val: number) => void;
  cartItems: any[];
}

export default function WholesaleProductPickerModal({ onClose, onAddToCart, onUpdateQty, cartItems }: WholesaleProductPickerModalProps) {
  const { wholesaleProducts, fetchWholesaleProducts, loading } = useWholesaleProductStore();
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-[40px] w-full max-w-4xl max-h-[85vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col">
        <div className="px-8 py-6 border-b border-gray-50 dark:border-neutral-800 flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-black">Add Wholesale Products</h3>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 bg-gray-100 dark:bg-neutral-800 rounded-md">Stock: {product.quantity}</span>
                  </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    <span className="text-sm font-black text-gray-900 dark:text-gray-100 block">₦{Number(product.price).toLocaleString()}</span>
                    <span className="text-[9px] text-gray-400 font-bold uppercase">Min: {product.min_order_quantity || 1}</span>
                  </div>
                  ...
                  {inCart ? (
                    <div className="flex items-center bg-gray-50 dark:bg-neutral-800 rounded-xl p-1 border border-gray-100 dark:border-neutral-700 animate-in fade-in zoom-in-95 duration-200">
                      <button 
                        onClick={() => onUpdateQty(product.id as any, -1)}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!isNaN(val)) {
                            onUpdateQty(product.id as any, val - inCart.quantity);
                          }
                        }}
